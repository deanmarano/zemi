---
title: GCP Cloud SQL - PostgreSQL Setup for Zemi
sidebar_label: GCP Cloud SQL
hide_title: true
description: How to configure your GCP Cloud SQL PostgreSQL database for Zemi change tracking using logical replication.
keywords: [Zemi, GCP Cloud SQL, PostgreSQL, Change Data Capture, logical replication, WAL]
---
...
Logical replication is turned off by default. To turn it on, update the [cloud flag](https://cloud.google.com/sql/docs/postgres/replication/configure-logical-replication#configure-your-postgresql-instance): `cloudsql.logical_decoding` = `on`. This requires a restart before `SHOW wal_level;` returns `logical`.

## Connection

Grant replication permission to your database user:

```sql
ALTER USER [user] WITH REPLICATION;
```

Then configure Zemi with the same credentials you use to connect from your application.
